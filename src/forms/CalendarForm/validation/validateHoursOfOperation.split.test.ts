...
test('A single-day closure splits correctly', () => {
  ...
  expect(split[Weekdays.Monday][0].row).toBe(0);
...
test('A non-filled entry does not split', () => {
  const split = splitRowsIntoWeekdays([
    {
      type: RowType.Closed,
      startDay: undefined,
...
test('A multi-day closure splits correctly', () => {
  ...
  expect(split[Weekdays.Sunday][0].row).toBe(0);
  ...
  expect(split[Weekdays.Friday][0].row).toBe(0);
  ...
  expect(split[Weekdays.Saturday][0].row).toBe(0);
...
test('A multi-day opening splits correctly', () => {
  ...
  expect(split[Weekdays.Monday][0].end.format('HH:mm')).toBe('23:59');
  expect(split[Weekdays.Monday]).toHaveLength(1);
  expect(split[Weekdays.Tuesday][0].row).toBe(0);
  ...
  expect(split[Weekdays.Wednesday][0].row).toBe(0);
...
test('A 24/7 opening splits correctly', () => {
  ...
  WEEKDAY_INDEX.forEach((weekday) => {
    ...
    expect(split[weekday][0].row).toBe(0);
...
test('A myriad of openings split correctly', () => {
  ...
  expect(split[Weekdays.Thursday][0].row).toBe(2);
