        ...
        if (event.exceptional === false) {
          if (today === event.openingDays[j].weekdays.day) {
            ...
            for (let k = 0; k < event.openingDays[j].openingDay.openingHour.length; k++) {
              ...
              if (
                event.openingDays[j].openingDay.openingHour.length > 1
                && event.openingDays[j].openingDay.openingHour.length > dates.length
              ) {
                dates.push(
                  <div>,</div>
                );
          ...
          eventContent = (
            <div
              ...
              style={{ border: '4px solid red', borderRadius: '4px' }}
            >
              {dates}
            </div>
          );
    ...
    for (let i = 0; i < this.state.openingAllPeriods.length; i++) {
      if (this.state.openingAllPeriods[i].id === event.id) {
        ...
        for (let j = 0; j < this.state.openingAllPeriods.length; j++) {
          if (
            filterEnd === this.state.openingAllPeriods[j].endDate
            && filterStart === this.state.openingAllPeriods[j].startDate
            && startTime === this.state.openingAllPeriods[j].openingDays[0].openingDay.openingHour[0].startTime
