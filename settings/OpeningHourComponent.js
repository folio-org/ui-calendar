...
const OpeningHourComponent = ({ fields }) =>
  (<div>
    {(fields || []).map((openingHour, index) => (
      <div key={index}>
        <Row>
          ...
          <Col xs={12} sm={2}>
            <Button fullWidth name="add" onClick={() => fields.push({ startHour: '0', startMinute: '0', endHour: '0', endMinute: '0' })}>Add</Button>
          </Col>
          <Col xs={12} sm={2}>
            <Button fullWidth name="remove" buttonStyle="danger" onClick={() => fields.remove(index)}>Remove</Button>
          </Col>
        </Row>
      </div>
    ))}
